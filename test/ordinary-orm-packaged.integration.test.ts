...
`);
      execute(process.execPath, [cli, 'run', entry, '--project-root', app, '--plan', 'alice,bob,alice,bob', '--max-runs', '1', '--timeout-ms', '30000', '--out', artifact, '--json'], app, 1);
      ...
      const repeated = parseRunArtifact(JSON.parse(execute(process.execPath, [cli, 'replay', entry, artifact, '--json'], app, 1)));
      ...
      const reduced = JSON.parse(execute(process.execPath, [cli, 'minimize', entry, artifact, '--max-attempts', '12', '--out', join(root, `${orm}-minimal.json`), '--json'], app, 1));
      ...
      const portable = parseRunArtifact(JSON.parse(execute(command, [...args, '--json'], destination, 1)));
