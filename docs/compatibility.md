...
The native fixture profile permits the built-in `plpgsql` extension. Other extensions, foreign relations, custom casts/operators, custom range/base types, temporary fixture state, logical replication configuration, and the other explicitly unsupported catalog features fail closed. Drivers other than node-postgres have separate qualification gates and are not covered by this matrix. At commit `8f273e5b4b55e99d57cb7d557a18b9d40e471499`, all six PostgreSQL 16/17/18 × Node.js 22.18.0/24.7.0 full-suite jobs passed, along with both PostgreSQL 17 / pgvector 0.8.6 catalog jobs and Chromium, Firefox and WebKit desktop/mobile reports. This matrix includes seeded search and CLI metrics, the Postgres.js protocol checks and the installed pghybrid workload with source-bound replay. See the [completed nine-job CI run](https://github.com/pavangupta352/interleave/actions/runs/34318463204).
